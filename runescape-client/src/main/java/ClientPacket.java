import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("mj")
@Implements("ClientPacket")
public class ClientPacket implements class321 {
	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJE;
	@ObfuscatedName("aw")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket FRIEND_CHAT_SETRANK;
	@ObfuscatedName("at")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket RESUME_COUNTDIALOG;
	@ObfuscatedName("ae")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPCE;
	@ObfuscatedName("ao")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOCE;
	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLAN_CHANNEL_KICK_USER;
	@ObfuscatedName("aj")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	static final ClientPacket field3392;
	@ObfuscatedName("av")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket EVENT_APPLET_FOCUS;
	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket FRIEND_CHAT_JOIN_LEAVE;
	@ObfuscatedName("an")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER4;
	@ObfuscatedName("af")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket RESUME_NAMEDIALOG;
	@ObfuscatedName("ai")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTONT;
	@ObfuscatedName("al")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPC4;
	@ObfuscatedName("ar")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLOSE_MODAL;
	@ObfuscatedName("as")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPHELDD;
	@ObfuscatedName("az")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLAN_CHANNEL_FULL_REQUEST;
	@ObfuscatedName("ak")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	static final ClientPacket field3332;
	@ObfuscatedName("ap")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IGNORE_DELUSER;
	@ObfuscatedName("ah")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket BUG_REPORT;
	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON4;
	@ObfuscatedName("ay")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLICK_WORLD_MAP;
	@ObfuscatedName("ax")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON6;
	@ObfuscatedName("aa")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJ1;
	@ObfuscatedName("aq")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON8;
	@ObfuscatedName("am")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLAN_SETTINGS_ADD_BANNED_FROM_CHANNEL;
	@ObfuscatedName("ag")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPCU;
	@ObfuscatedName("bw")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPCT;
	@ObfuscatedName("bm")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPC3;
	@ObfuscatedName("bv")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MAP_BUILD_COMPLETE;
	@ObfuscatedName("bt")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJ4;
	@ObfuscatedName("bk")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket field3346;
	@ObfuscatedName("bb")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJU;
	@ObfuscatedName("ba")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket EVENT_WINDOW_SETTING;
	@ObfuscatedName("bp")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJ3;
	@ObfuscatedName("bo")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket RESUME_PAUSEBUTTON;
	@ObfuscatedName("bu")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket REFLECTION_CHECK_REPLY;
	@ObfuscatedName("bj")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON1;
	@ObfuscatedName("bh")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER5;
	@ObfuscatedName("bg")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJ2;
	@ObfuscatedName("bl")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOC2;
	@ObfuscatedName("bi")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CHAT_SENDABUSEREPORT;
	@ObfuscatedName("bz")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON10;
	@ObfuscatedName("bx")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOC5;
	@ObfuscatedName("br")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MINIMAP_CLICK;
	@ObfuscatedName("bq")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER3;
	@ObfuscatedName("bn")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket EVENT_CAMERA_POSITION;
	@ObfuscatedName("bd")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYERT;
	@ObfuscatedName("bf")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	static final ClientPacket field3366;
	@ObfuscatedName("be")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket RESUME_STRINGDIALOG;
	@ObfuscatedName("by")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER6;
	@ObfuscatedName("bs")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CHAT_SETFILTER;
	@ObfuscatedName("bc")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLAN_SETTINGS_SET_MUTED_FROM_CHANNEL;
	@ObfuscatedName("cc")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOC3;
	@ObfuscatedName("ct")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER2;
	@ObfuscatedName("cy")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJT;
	@ObfuscatedName("cv")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYERU;
	@ObfuscatedName("cq")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket RESUME_OBJDIALOG;
	@ObfuscatedName("cn")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON3;
	@ObfuscatedName("ck")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLAN_KICKUSER;
	@ObfuscatedName("cs")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CLAN_SETTINGS_FULL_REQUEST;
	@ObfuscatedName("co")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOCU;
	@ObfuscatedName("ci")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPC1;
	@ObfuscatedName("cl")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket EVENT_MOUSE_IDLE;
	@ObfuscatedName("ca")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket CHAT_SENDPRIVATE;
	@ObfuscatedName("cd")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MESSAGE_PUBLIC;
	@ObfuscatedName("cb")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOC4;
	@ObfuscatedName("ce")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket DETECT_MODIFIED_CLIENT;
	@ObfuscatedName("cw")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	static final ClientPacket field3371;
	@ObfuscatedName("cj")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket UPDATE_PLAYER_MODEL;
	@ObfuscatedName("cu")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket field3387;
	@ObfuscatedName("cm")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket WIDGET_TYPE;
	@ObfuscatedName("ch")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_SUBOP;
	@ObfuscatedName("cg")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MOVE_GAMECLICK;
	@ObfuscatedName("cp")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON2;
	@ObfuscatedName("cr")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	static final ClientPacket field3390;
	@ObfuscatedName("cz")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket EVENT_KEYBOARD;
	@ObfuscatedName("cx")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON9;
	@ObfuscatedName("cf")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER7;
	@ObfuscatedName("df")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket field3394;
	@ObfuscatedName("dg")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket NO_TIMEOUT;
	@ObfuscatedName("dq")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPOBJ5;
	@ObfuscatedName("dt")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOCT;
	@ObfuscatedName("dh")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket FRIEND_DELUSER;
	@ObfuscatedName("dr")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket PING_STATISTICS;
	@ObfuscatedName("dk")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER8;
	@ObfuscatedName("dd")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket EVENT_MOUSE_CLICK;
	@ObfuscatedName("dz")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPLOC1;
	@ObfuscatedName("dc")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON7;
	@ObfuscatedName("dl")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket DOCHEAT;
	@ObfuscatedName("do")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPPLAYER1;
	@ObfuscatedName("dw")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_CRMVIEW;
	@ObfuscatedName("dp")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MOUSE_MOVE;
	@ObfuscatedName("dn")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MUSIC_PLAYING;
	@ObfuscatedName("de")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket MOUSE_WHEEL;
	@ObfuscatedName("dy")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPC5;
	@ObfuscatedName("dj")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket field3411;
	@ObfuscatedName("dx")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket OPNPC2;
	@ObfuscatedName("du")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket IF_BUTTON5;
	@ObfuscatedName("da")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket SET_HEADING;
	@ObfuscatedName("di")
	@ObfuscatedSignature(
		descriptor = "Lmj;"
	)
	public static final ClientPacket LOGIN_TIMINGS;
	@ObfuscatedName("ds")
	@ObfuscatedGetter(
		intValue = 611244161
	)
	@Export("id")
	final int id;
	@ObfuscatedName("dv")
	@ObfuscatedGetter(
		intValue = -1968664625
	)
	@Export("length")
	final int length;

	static {
		OPOBJE = new ClientPacket(0, 6);
		FRIEND_CHAT_SETRANK = new ClientPacket(1, -1);
		RESUME_COUNTDIALOG = new ClientPacket(2, 4);
		OPNPCE = new ClientPacket(3, 2);
		OPLOCE = new ClientPacket(4, 2);
		CLAN_CHANNEL_KICK_USER = new ClientPacket(5, -1);
		field3392 = new ClientPacket(6, -1);
		EVENT_APPLET_FOCUS = new ClientPacket(7, 1);
		FRIEND_CHAT_JOIN_LEAVE = new ClientPacket(8, -1);
		OPPLAYER4 = new ClientPacket(9, 3);
		RESUME_NAMEDIALOG = new ClientPacket(10, -1);
		IF_BUTTONT = new ClientPacket(11, 16);
		OPNPC4 = new ClientPacket(12, 3);
		CLOSE_MODAL = new ClientPacket(13, 0);
		OPHELDD = new ClientPacket(14, 16);
		CLAN_CHANNEL_FULL_REQUEST = new ClientPacket(15, 1);
		field3332 = new ClientPacket(16, 1);
		IGNORE_DELUSER = new ClientPacket(17, -1);
		BUG_REPORT = new ClientPacket(18, -2);
		IF_BUTTON4 = new ClientPacket(19, 8);
		CLICK_WORLD_MAP = new ClientPacket(20, 4);
		IF_BUTTON6 = new ClientPacket(21, 8);
		OPOBJ1 = new ClientPacket(22, 7);
		IF_BUTTON8 = new ClientPacket(23, 8);
		CLAN_SETTINGS_ADD_BANNED_FROM_CHANNEL = new ClientPacket(24, -1);
		OPNPCU = new ClientPacket(25, 11);
		OPNPCT = new ClientPacket(26, 11);
		OPNPC3 = new ClientPacket(27, 3);
		MAP_BUILD_COMPLETE = new ClientPacket(28, 0);
		OPOBJ4 = new ClientPacket(29, 7);
		field3346 = new ClientPacket(30, -1);
		OPOBJU = new ClientPacket(31, 15);
		EVENT_WINDOW_SETTING = new ClientPacket(32, 5);
		OPOBJ3 = new ClientPacket(33, 7);
		RESUME_PAUSEBUTTON = new ClientPacket(34, 6);
		REFLECTION_CHECK_REPLY = new ClientPacket(35, -1);
		IF_BUTTON1 = new ClientPacket(36, 8);
		OPPLAYER5 = new ClientPacket(37, 3);
		OPOBJ2 = new ClientPacket(38, 7);
		OPLOC2 = new ClientPacket(39, 7);
		CHAT_SENDABUSEREPORT = new ClientPacket(40, -1);
		IF_BUTTON10 = new ClientPacket(41, 8);
		OPLOC5 = new ClientPacket(42, 7);
		MINIMAP_CLICK = new ClientPacket(43, -1);
		OPPLAYER3 = new ClientPacket(44, 3);
		EVENT_CAMERA_POSITION = new ClientPacket(45, 4);
		OPPLAYERT = new ClientPacket(46, 11);
		field3366 = new ClientPacket(47, 2);
		RESUME_STRINGDIALOG = new ClientPacket(48, -1);
		OPPLAYER6 = new ClientPacket(49, 3);
		CHAT_SETFILTER = new ClientPacket(50, 3);
		CLAN_SETTINGS_SET_MUTED_FROM_CHANNEL = new ClientPacket(51, -1);
		OPLOC3 = new ClientPacket(52, 7);
		OPPLAYER2 = new ClientPacket(53, 3);
		OPOBJT = new ClientPacket(54, 15);
		OPPLAYERU = new ClientPacket(55, 11);
		RESUME_OBJDIALOG = new ClientPacket(56, 2);
		IF_BUTTON3 = new ClientPacket(57, 8);
		CLAN_KICKUSER = new ClientPacket(58, -1);
		CLAN_SETTINGS_FULL_REQUEST = new ClientPacket(59, 1);
		OPLOCU = new ClientPacket(60, 15);
		OPNPC1 = new ClientPacket(61, 3);
		EVENT_MOUSE_IDLE = new ClientPacket(62, 0);
		CHAT_SENDPRIVATE = new ClientPacket(63, -2);
		MESSAGE_PUBLIC = new ClientPacket(64, -1);
		OPLOC4 = new ClientPacket(65, 7);
		DETECT_MODIFIED_CLIENT = new ClientPacket(66, 4);
		field3371 = new ClientPacket(67, -1);
		UPDATE_PLAYER_MODEL = new ClientPacket(68, 26);
		field3387 = new ClientPacket(69, 0);
		WIDGET_TYPE = new ClientPacket(70, 4);
		IF_SUBOP = new ClientPacket(71, 10);
		MOVE_GAMECLICK = new ClientPacket(72, -1);
		IF_BUTTON2 = new ClientPacket(73, 8);
		field3390 = new ClientPacket(74, 7);
		EVENT_KEYBOARD = new ClientPacket(75, -2);
		IF_BUTTON9 = new ClientPacket(76, 8);
		OPPLAYER7 = new ClientPacket(77, 3);
		field3394 = new ClientPacket(78, -1);
		NO_TIMEOUT = new ClientPacket(79, 0);
		OPOBJ5 = new ClientPacket(80, 7);
		OPLOCT = new ClientPacket(81, 15);
		FRIEND_DELUSER = new ClientPacket(82, -1);
		PING_STATISTICS = new ClientPacket(83, 10);
		OPPLAYER8 = new ClientPacket(84, 3);
		EVENT_MOUSE_CLICK = new ClientPacket(85, 6);
		OPLOC1 = new ClientPacket(86, 7);
		IF_BUTTON7 = new ClientPacket(87, 8);
		DOCHEAT = new ClientPacket(88, -1);
		OPPLAYER1 = new ClientPacket(89, 3);
		IF_CRMVIEW = new ClientPacket(90, 22);
		MOUSE_MOVE = new ClientPacket(91, -1);
		MUSIC_PLAYING = new ClientPacket(92, 4);
		MOUSE_WHEEL = new ClientPacket(93, 2);
		OPNPC5 = new ClientPacket(94, 3);
		field3411 = new ClientPacket(95, 9);
		OPNPC2 = new ClientPacket(96, 3);
		IF_BUTTON5 = new ClientPacket(97, 8);
		SET_HEADING = new ClientPacket(98, 1);
		LOGIN_TIMINGS = new ClientPacket(99, -1);
	}

	ClientPacket(int var1, int var2) {
		this.id = var1;
		this.length = var2;
	}
}
